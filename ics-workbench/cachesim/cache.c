...
uint32_t cache_read(uintptr_t addr) {
  int offset=BITS(addr & ~0x3,offset_width-1,0);
  int idx=BITS(addr,offset_width+idx_width-1,offset_width);
  int tag=BITS(addr,ADDR_WIDTH-1,offset_width+idx_width);
  for(int i=0;i<way;i++){
    if(cache_tag[i][idx]==tag&&V[i][idx]){//hit
      hit_cnt++;
  ...
  //dirty
  if(D[way2][idx]){
    mem_write(addr>>BLOCK_WIDTH,cache_data[way2][idx]);
    D[way2][idx]=0;
  }
