...
static word_t immB(uint32_t i) {printf("%x %llx %llx %llx\n",i,(BITS(i, 30, 25)<< 5),(BITS(i, 11, 8) << 1),(BITS(i, 7, 7)) << 11); return (SEXT(BITS(i, 31, 31), 1) << 12)| (SEXT(BITS(i, 30, 25), 6) << 5)|(SEXT(BITS(i, 11, 8), 4) << 1)|(SEXT(BITS(i, 7, 7), 1) << 11);}
...
static void decode_operand(Decode *s, word_t *dest, word_t *src1, word_t *src2, int type) {
  ...
  switch (type) {
    ...
    case TYPE_B: destI(immB(i)); printf("%x\n\n",i);src1R(rs1); src2R(rs2); break;
