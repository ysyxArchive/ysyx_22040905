...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
...
#ifdef CONFIG_FTRACE	  
	 if(dest==0&&srs1==1&&src2==0)ftrace_add(s->pc,s->dnpc,0);else ftrace_add(s->pc,s->dnpc,1)
#endif
	 );
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw   , R, printf("%lld %ld\n",BITS(src1,31,0),src2);R(dest) = SEXT((int32_t)(BITS(src1,31,0))%(int32_t)(BITS(src2,31,0)),32));
  ...
  INSTPAT("010000? ????? ????? 101 ????? 00110 11", sraiw  , I, R(dest) = SEXT((int32_t)BITS(src1,31,0)>>src2,32));
  ...
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(dest) = BITS(src1*src2,63,32));
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(dest) = BITS((int64_t)src1*src2,63,32));
