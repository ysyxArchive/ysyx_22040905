...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 101 ????? 00100 11", srli   , I, printf("r:%ld %lld\n",src1,(BITS(src2,4,0))); R(dest) = (src1>>src2));
  INSTPAT("??????? ????? ????? 001 ????? 00100 11", slli   , I, printf("l:%ld %ld %lld\n",src1<<(BITS(src2,4,0)),src1,(BITS(src2,4,0))); R(dest) = (src1<<src2));
