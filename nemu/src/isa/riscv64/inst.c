...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = s->pc+4,s->pc+=src1);////printf("%ld\n",s->pc);s->pc+=src1;printf("%ld\n",s->pc));
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(dest) = src1);
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(dest) = src1<<src2);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, vaddr_t t=s->pc+4;s->pc=((src1+src2)&(-1)),R(dest) = t);
