...
class AXI4Arbiter extends Module{
  ...
  //arstate := MuxLookup(arstate, s_idle, List(
  //s_idle  ->  Mux(io.lsu.ar.valid,s_lsu,
  //            Mux(io.ifu.ar.valid,s_ifu,s_idle)),
  //s_ifu   ->  Mux(io.ifu.ar.fire, s_idle, s_ifu),
  //s_lsu   ->  Mux(io.lsu.ar.fire, s_idle, s_lsu)
  //))

  //rstate := MuxLookup(rstate, s_idle, List(
  //  s_idle -> Mux(~io.out.r.valid,s_idle,
  //            Mux(io.out.r.bits.id === 1.U,s_lsu,s_ifu)),
  //  s_ifu  -> Mux(io.out.r.fire,s_idle,s_ifu),
  //  s_lsu  -> Mux(io.out.r.fire,s_idle,s_lsu)
  //))
