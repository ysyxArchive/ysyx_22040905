...
class CSR extends Module{
    ...
    clint_irq :=  csr(0)(3)  & csr(4)(7)  & io.clint.mtip(0)
    ...
    csr(0):=Mux((io.w.no===3.U)|(io.w.no===1.U),Cat(Cat(Cat(Cat(csr(0)(63,8),csr(0)(3)),csr(0)(6,4)),0.U(1.W)),csr(0)(2,0)), //• MPIE设为MIE的值，MIE设为0
            Mux(io.w.no===2.U,Cat(Cat(Cat(Cat(csr(0)(63,8),1.U(1.W)),csr(0)(6,4)),csr(0)(7)),csr(0)(2,0)), //MIE从MPIE中恢复，MPIE设置为1
            csr(0)))
    csr(2):=Mux((io.w.no===3.U)|(io.w.no===1.U),io.w.epc,csr(2))
    csr(3):=Mux(io.w.no===3.U,"x8000000000000007".U,
