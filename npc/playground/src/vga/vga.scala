...
class vmem extends Module{
    ...
    val ram=RegInit(VecInit(Seq.fill(16)(0.U(90.W)))) 
    ...
    when(io.now=/=0.U){         
        for (i <- 0 to 15){
        ram(h+i.asUInt):=ram(h+i.asUInt)^Cat(Cat(Fill(v.asSInt*9.S,0.U),vga_mem(16.U*io.ascii)(0,8),Fill(90.S-v.asSInt*9.S-9.S,0.U)))
        }
        v:=v+9.U
    }
